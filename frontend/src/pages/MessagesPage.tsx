...
import {
  ...
  useTheme, useMediaQuery, alpha, Badge, ClickAwayListener, Slider, CircularProgress,
  Menu, MenuItem, Checkbox, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button
} from '@mui/material';
...
function AudioPlayer({ msg, isDark }: { msg: Message; isDark: boolean }) {
  ...
  // Initialize audio element if audioUrl exists
  useEffect(() => {
  ...
  useEffect(() => {
    if (!msg.audioUrl) {
      // Mock playback logic for simulated voice notes
      if (isPlaying) {
        const step = 0.1; // update every 100ms
  ...
  return (
    ...
    >
      ...
      {/* Profile avatar / Mic visual representation */}
...
// Component rendering individual chat bubbles
const Bubble = ({ msg, selected, onContextMenu, isDark, selectionMode, onToggle, onNavigateProfile }: { msg: Message; isDark: boolean; onContextMenu: (e: React.MouseEvent, msgId: string) => void; selected?: boolean; selectionMode?: boolean; onToggle?: (msgId: string) => void; onNavigateProfile?: (contactId: string) => void; }) => {
  const handleClick = () => {
  ...
  if (msg.isDeleted) {
    return (
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: '0.875rem' }}>🚫 Ce message a été supprimé</Typography>
          <Typography sx={{ fontSize: '0.6875rem', alignSelf: 'flex-end', ml: 1, color: isDark ? 'rgba(233,237,239,0.5)' : 'rgba(17,27,33,0.45)' }}>
            {msg.time}
          </Typography>
        </Box>
      </Box>
  ...
  if (msg.isDocument) {
    return (
      ...
      >
        ...
        >
          <Box sx={{ px: 1, pt: 1 }}>{renderReplyPreview()}</Box>
...
function ConvoItem({ conv, active, onClick }: { conv: Conversation; active: boolean; onClick: () => void }) {
...
export default function MessagesPage() {
  ...
  const [contextMenu, setContextMenu] = useState<{ mouseX: number; mouseY: number; msgId: string | null } | null>(null);

  const hasFetched = useRef(false);

  // Synchronize conversations unread count with Redux dynamically
  useEffect(() => {
  ...
  // Polling for new messages (Real-time WhatsApp like)
  useEffect(() => {
          ...
          setConversations(prev => {
            ...
            res.data.forEach((convo: any) => {
              ...
              if (existingIdx >= 0) {
                ...
                // If we found new messages OR if we replaced some simulated IDs, update state
                ...
                if (newIncoming.length > 0 || idsChanged) {
                  ...
                  if (existingC.id === activeId) {
                    dispatch(markAsRead(activeId)); // mark read immediately if open
                  }
                }
              } else {
  ...
  // Apply special viewport only on this page to handle mobile keyboard properly
  useEffect(() => {
  ...
  // Format recording seconds (e.g. 5 -> "0:05")
  ...
  // Stop recording and send audio voice note
  const sendVoiceNote = () => {
    if (!activeId) return;
    setIsRecording(false);
    ...
    if (mediaRecorderRef.current) {
      // Capture the current recordSeconds before we lose it
  ...
  const handleContextMenu = (event: React.MouseEvent, msgId: string) => {
    ...
    setContextMenu(
      contextMenu === null
        ? { mouseX: event.clientX + 2, mouseY: event.clientY - 6, msgId }
        : null,
    );
  };
  ...
  const handleCopyMessage = () => {
    if (contextMenu?.msgId && activeConv) {
      const msg = activeConv.messages.find(m => m.id === contextMenu.msgId);
  ...
  const handleBulkDelete = () => {
    if (selectedMessages.length > 0) {
      setConfirmDelete({
        ...
        type: 'bulk',
        action: () => {
          dispatch(bulkDeleteMessages(selectedMessages));
          setConversations(prev => prev.map(c => c.id === activeId ? {
            ...c, messages: c.messages.map(m => selectedMessages.includes(m.id) ? { ...m, isDeleted: true } : m)
          } : c));
          setSelectedMessages([]);
    ...
    >
          ...
          {activeConv ? (
            <>
              ...
              >
                ...
                {selectionMode ? (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography variant="body2" sx={{ fontWeight: 600, mr: 1, color: theme.palette.primary.main }}>
                      {selectedMessages.length} sélectionné(s)
                    </Typography>
                    <IconButton size="small" onClick={handleBulkDelete} color="error" disabled={selectedMessages.length === 0} sx={{ borderRadius: '8px' }}>
                      <DeleteIcon sx={{ fontSize: 20 }} />
                    </IconButton>
                    <IconButton size="small" onClick={() => { setSelectionMode(false); setSelectedMessages([]); }} sx={{ borderRadius: '8px' }}>
                      <Typography variant="caption" sx={{ fontWeight: 600 }}>Annuler</Typography>
                    </IconButton>
                  </Box>
                ) : (
                  <>
                    <IconButton size="small" onClick={() => { setSelectionMode(true); setSelectedMessages([]); }} sx={{ borderRadius: '8px', mr: -1 }}>
                      <CheckBoxOutlineBlankIcon sx={{ fontSize: 20, color: 'text.secondary' }} />
                    </IconButton>
                    <IconButton size="small" onClick={handleDeleteConversation} className="pressable" sx={{ borderRadius: '8px' }}>
                      <DeleteIcon sx={{ fontSize: 20, color: 'text.secondary' }} />
                    </IconButton>
                    <IconButton size="small" className="pressable" sx={{ borderRadius: '8px' }}>
                ...
                {activeConv.messages.map((msg) => (
                  <Bubble
                    ...
                    onNavigateProfile={(id) => {
                      const basePath = currentRole === 'admin' ? '/admin/search' : '/search';
                      routerNavigate(`${basePath}/${id}`);
                    }}
                  />
                ))}
              ...
              >
                <MenuItem onClick={() => { setSelectionMode(true); toggleSelection(contextMenu?.msgId || ''); handleCloseContext(); }} sx={{ gap: 1.5, fontSize: '0.9rem' }}>
                  <CheckBoxOutlineBlankIcon fontSize="small" /> Sélectionner
                </MenuItem>
                <MenuItem onClick={handleCopyMessage} sx={{ gap: 1.5, fontSize: '0.9rem' }}>
                  <ContentCopyIcon fontSize="small" /> Copier
                </MenuItem>
                <MenuItem onClick={handleDeleteMessage} sx={{ gap: 1.5, fontSize: '0.9rem', color: 'error.main' }}>
                  <DeleteIcon fontSize="small" color="error" /> Supprimer
                </MenuItem>
              </Menu>
      ...
      >
        <MenuItem onClick={handleReplyMessage}>
          <ReplyIcon sx={{ mr: 1, fontSize: 20 }} /> Répondre
        </MenuItem>
        <MenuItem onClick={handleDeleteMessage} sx={{ color: 'error.main' }}>
          <DeleteIcon sx={{ mr: 1, fontSize: 20 }} /> Supprimer pour tous
        </MenuItem>
      </Menu>
      ...
      <ContactSearchModal
        ...
        onSelect={(user) => {
          ...
          setConversations((prev) => {
            const updated = prev.map((c) =>
              c.id === activeId
                ? { ...c, messages: [...c.messages, newMsg], lastMsg: `👤 Contact`, time }
                : c
            );
            const idx = updated.findIndex((c) => c.id === activeId);
            if (idx > 0) {
          ...
          api.post('messages/', { conversation: activeId, text: contactStr })
            .then((res) => {
              setConversations(prev => prev.map(c => c.id === activeId ? {
                ...c, messages: c.messages.map(m => m.id === newMsg.id ? { ...m, id: String(res.data.id) } : m)
              } : c));
            })
            .catch(err => console.error('Contact upload error', err));
        }}
      />
